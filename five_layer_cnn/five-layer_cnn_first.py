...
tf.app.flags.DEFINE_string('coverdirectory', '',"""The path of cover images封面图片""")
tf.app.flags.DEFINE_string('stegdirectory', '',"""The path of stego images隐写图片""")
tf.app.flags.DEFINE_string('stegodir_test', '',"""The path of test images.测试图片""")
tf.app.flags.DEFINE_string('network', '',"""The path of Pre-trained network.预训练网络""")
...
##############padding=same 对图像进行填充 但是最终输出和输入尺度结果一样        padding=valid 不允许对图像进行填充
##### 1 第一层卷积，5*5的卷积核大小，1的原始特征→8的特征图  5*5的池化尺寸 2的池化步长 
f_conv1 = my_conv_layer(
    z_c,  # 图片信息矩阵（输入纬度=输出纬度）
    5, 5,  # 卷积核大小
    1,     # 输入特征图大小
    8,     # 输出特征图大小
    5,     # 池化尺寸为5 * 5 
    2,     # 池化步长
    1,     # 激活函数tanh 1 或relu 0 （带BN）
    1,     # 激活ABS 1
    'SAME' # padding
)
f_conv1_shape = f_conv1.get_shape().as_list()

f_conv2 = my_conv_layer(f_conv2,5,5,8,16,5,2,1,0,'SAME')
f_conv2_shape = f_conv2.get_shape().as_list()

f_conv3 = my_conv_layer(f_conv3,1,1,16,32,5,2,1,0,'SAME')
f_conv3_shape = f_conv3.get_shape().as_list()

f_conv4 = my_conv_layer(f_conv4,1,1,32, 64,5,2,0,0,'SAME')
f_conv4_shape = f_conv4.get_shape().as_list()

f_conv5 = my_conv_layer(f_conv5,1,1,64,128,5,2,0,0,'SAME')
f_conv5_shape = f_conv5.get_shape().as_list()

f_conv6 = my_conv_layer(f_conv6,1,1,128,256,16,1,0,0,'VALID')
...
##################  重塑    全连接层       softmax层  
f_conv_shape = f_conv6.get_shape().as_list()
f_conv = tf.reshape(f_conv6,[f_conv_shape[0],f_conv_shape[1]*f_conv_shape[2]*f_conv_shape[3]])
...
# Training
train_step = tf.train.MomentumOptimizer(learning_rate=1e-5,momentum=0.9).minimize(cross_entropy)
                                        ##学习率0.000005   稳定参数momentum=0.9 可选还有momentum量因子   epsilon常数    decay学习衰减率
