...
def load_cfar10_batch(cifar10_dataset_folder_path, batch_id):
    ...
    labels = batch['labels']
    return features[:8000], labels[:8000]


# 加载所有训练数据
cifar10_path = 'R:/workspaces_files/tensor_resorce/demo_origin/batches'  # 本地路径
# 共有5个batch的训练数据
...
conv2 = tf.layers.conv2d(
    conv1, 128, (4, 4), padding='same', activation=tf.nn.relu,
    kernel_initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.1))
# 16 x 16 x 128 to 8 x 8 x 128
conv2 = tf.layers.max_pooling2d(conv2, (2,2), (2,2), padding='same')

# 重塑输出
shape = np.prod(conv2.get_shape().as_list()[1:])  # 获取conv2维度，转为列表取值
conv2 = tf.reshape(conv2,[-1, shape])  # 按一维展开，-1：表示reshape时，该位置的大小自动获取，以便使总大小不变

# 第一层全连接层
# 8 x 8 x 128 to 1 x 1024
fc1 = tf.contrib.layers.fully_connected(conv2, 1024, activation_fn=tf.nn.relu)  # 用于构建神经网络层、正则化器、摘要等的操作.
...
logits_ = tf.identity(logits_, name='logits_')  # identify：输出与输入具有相同形状和内容的张量。将操作和名称关联

# cost & optimizer
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits_, labels=targets_))
# optimizer = tf.train.AdamOptimizer(learning_rate=0.001).minimize(cost)
optimizer = tf.train.AdamOptimizer(learning_rate=1e-4, epsilon=1e-5).minimize(cost)
...
save_model_path = 'R:/workspaces_files/tensor_resorce/demo_origin/modelgpu/test_cifar'
count = 0
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        for batch_i in range(img_shape[0] // batch_size - 1):
            ...
            if (count % 10 == 0):
                print('{}: Epoch {:>2}, Train Loss {:.4f}, Validation Accuracy {:4f} '.format(
    ...
    print('{}: finishing training'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())))
